...
import { Category, Product, categories as starterCategories, connectDB } from './service.js';
...
export async function POST(req) {
  try {
    ...
    if (error.code === 11000) {
      return json({ message: 'Category slug already exists' }, { status: 409 });
    }
    return json({ message: 'Internal Server Error', error: error.message }, { status: 500 });
  }
}
...
export async function PUT(req) {
  try {
    ...
    if (!categoryId || !name || !slug) {
      return json({ message: 'Category id, name, and slug are required' }, { status: 400 });
    }
...
export async function DELETE(req) {
  try {
    ...
    if (category.slug === UNCATEGORIZED.slug) {
      return json({ message: 'Uncategorized cannot be deleted' }, { status: 400 });
    }
